...
这是因为我们已经提前知道了Qemu 模拟的 RISC-V virt 计算机中的物理内存布局，Qemu 规定的 DRAM 物理内存的起始物理地址为 `0x80000000`，而默认RAM大小为`128MiB`，因此，默认的 DRAM 物理内存地址范围就是 `[0x80000000,0x88000000)`

但是这种硬编码获取可用物理内存的可移植性不强，即只能在某一特定的机器上运行。

如果我们想拓展代码的可移植性，就需要动态的获取硬件的可用物理内存范围。

在 RISC-V 中，一般由OpenSBI 来告知操作系统可用的物理内存范围，它来完成对于包括物理内存在内的各外设的扫描，将扫描结果以 DTB(Device Tree Blob) 的格式保存在物理内存中的某个地方。随后 OpenSBI 会将其地址保存在 `a1` 寄存器中，供操作系统使用。

当操作系统内核开始引导时，它会从 a1寄存器处接收设备树的地址。内核会在初始化过程中解析设备树，以识别和配置硬件资源。设备树的结构是树形的，包含节点和属性。每个节点代表一个设备或资源（例如物理内存），而属性则包含设备的详细信息。操作系统会从 DTB 的根节点开始，遍历树形结构，查找与物理内存相关的节点，一旦找到相关节点，就会读取节点属性来获取物理内存的起始地址和大小。

由于OpenSBI本身会占用一部分物理内存，这一部分内存是不能被操作系统使用的，而OpenSBI会在设备树中标记其占用的内存为保留内存(reserved memory)，故操作系统也需要查找这一部分物理内存的地址，并将其标记为保留内存。

除去OpenSBI占用的内存外，剩下的内存即为可用的物理内存。
